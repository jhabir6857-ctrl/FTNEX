...
| Animation | GSAP + ScrollTrigger + `@gsap/react` (`useGSAP()`) | Drives hero canvas + secondary micro-interactions; `useGSAP()` required for proper cleanup on route change (see Section 6) |
...
- Explicitly treat the **poster/placeholder image** as the LCP element (it should be the first meaningful paint), not the fully-loaded canvas sequence — set this via priority loading (`next/image` with `priority`, or eager-loaded `<img>` for the poster).
  - This `priority` flag should be applied to the poster image **only**. Sprite sheets and later frames must not be marked priority — doing so competes with the poster for bandwidth during initial load and defeats the point of protecting LCP.
...
### Animation implementation constraints (required, not optional)
- **Overlay text/UI animations** (headline, subhead, any DOM element layered over the canvas) must animate only `transform` (`translate`, `scale`) and `opacity`. Do not animate `top`, `left`, `height`, `width`, or `margin` during scroll-scrubbed sequences — these trigger layout recalculation on every scroll tick and will cause visible frame drops, defeating the point of the 12–15fps canvas budget in Section 6.
- **Mounting/cleanup:** Use the `@gsap/react` `useGSAP()` hook to initialize GSAP timelines and `ScrollTrigger` instances in `ScrollCanvas.tsx` (and any other component with scroll-tied animation), not a raw `useEffect`. `useGSAP()`'s automatic context revert on unmount ensures `ScrollTrigger` instances and their scroll listeners are fully torn down when the user navigates to `/about`, `/services`, etc. This matters more than usual here because Next.js App Router persists client state across route transitions rather than hard-reloading — a manually-cleaned-up `useEffect` is an easy place to leak a zombie scroll listener.
